...
proc tablelist::defineTablelistBody {} {
	...
	if {[winfo exists %W]} {
		...
		tablelist::condBeginMove $tablelist::W $tablelist::priv(row) $tablelist::priv(col)
...
# browse or extended.
#------------------------------------------------------------------------------
proc tablelist::condBeginMove {win row col} {
    upvar ::tablelist::ns${win}::data data
...
proc tablelist::motion {win row col} {
	...
	row {
		...
		extended {
		    # only extend selection when we are not
		    # currently moving a row
		    if {![info exists priv(moving)]} {
